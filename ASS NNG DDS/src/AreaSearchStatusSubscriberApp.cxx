...
/*!
 * @file AreaSearchStatusSubscriberApp.cxx
...
AreaSearchStatusSubscriberApp::AreaSearchStatusSubscriberApp(
        const int& domain_id)
    : factory_(nullptr)
    ...
    , stop_(false)
{
    ...
    pqos.name("test_msgs_pkg::msg::AreaSearchStatus_sub_participant");
    ...
    {
        throw std::runtime_error("test_msgs_pkg::msg::AreaSearchStatus Subscriber initialization failed");
    }
    ...
    if (reader_ == nullptr)
    {
        throw std::runtime_error("test_msgs_pkg::msg::AreaSearchStatus DataReader initialization failed");
    }
...
void AreaSearchStatusSubscriberApp::on_subscription_matched(
        DataReader* /*reader*/,
        const SubscriptionMatchedStatus& info)
{
    if (info.current_count_change == 1)
    {
        std::cout << "test_msgs_pkg::msg::AreaSearchStatus Subscriber matched." << std::endl;
    }
    else if (info.current_count_change == -1)
    {
        std::cout << "test_msgs_pkg::msg::AreaSearchStatus Subscriber unmatched." << std::endl;
    }
    else
    {
...
void AreaSearchStatusSubscriberApp::on_data_available(
        DataReader* reader)
{
    ...
    while ((!is_stopped()) && (RETCODE_OK == reader->take_next_sample(&sample_, &info)))
    {
        if ((info.instance_state == ALIVE_INSTANCE_STATE) && info.valid_data)
        {
            ...
            std::cout << "Name: " << sample_.vehicle_id() << "\n";
...
void AreaSearchStatusSubscriberApp::run()
{
...
bool AreaSearchStatusSubscriberApp::is_stopped()
{
    return stop_.load();
}

void AreaSearchStatusSubscriberApp::stop()
{
    stop_.store(true);
